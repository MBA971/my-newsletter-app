...
const AdminView = ({
    ...
    // Refresh data when component mounts or when currentUser changes
    useEffect(() => {
        // This will ensure data is refreshed when the component mounts
        // and when the currentUser changes
    }, [currentUser]);
    ...
    // Dashboard Stats
    const getStats = (currentUser, domains, users, news) => {
        // For domain admins, filter data to their domain
        if (currentUser.role === 'domain_admin') {
            const domainNews = news.filter(item => item.domain === currentUser.domain);
            // Filter users to only include contributors and domain admins in this domain
            const domainUsers = users.filter(user => 
                user.domain === currentUser.domain && 
                (user.role === 'contributor' || user.role === 'domain_admin')
            );
            const domainDomains = domains.filter(domain => domain.name === currentUser.domain);
            ...
            return [
                ...
                {
                    label: 'Users in Domain',
                    value: domainUsers.length,
                    icon: Users,
                    gradientClass: 'text-gradient-purple',
                    bgClass: 'bg-purple-50'
                },
                {
                    label: 'Articles in Domain',
                    value: domainNews.length,
                    icon: FileText,
    ...
    return (
        <div className="animate-fadeIn">
            ...
            <div className="grid grid-cols-4 gap-6 mb-8">
                {getStats(currentUser, domains, users, news).map((stat, index) => (
